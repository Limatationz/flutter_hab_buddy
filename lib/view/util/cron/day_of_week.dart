...
class DayOfWeek extends CronEntity implements CronPart {
  ...
  @override
  String toReadableString2(BuildContext context) {
    switch (cronDay.type) {
      case CronDayType.EVERY_WEEK:
        return '';
      case CronDayType.EVERY_MONTH:
  ...
  String toReadableString(BuildContext context) {
    switch (cronDay.type) {
      case CronDayType.EVERY_WEEK:
        return S.of(context).cronEveryWeek;
      case CronDayType.EVERY_MONTH:
        return ''; // Handle if required
      case CronDayType.EVERY_START_AT_WEEK:
        var startDay = everyStartDay ?? 0;
        return startDay > 0
            ? S.of(context).cronEveryDayInWeekStartingOn(
                everyDay, serialNumberName(startDay))
            : S.of(context).cronEveryDay(everyDay);
      case CronDayType.EVERY_START_AT_MONTH:
        return ''; // Handle if required
      case CronDayType.SPECIFIC_DAY_OF_WEEK:
        var days = (specificWeekdays.isEmpty ? [startIndex] : specificWeekdays)
            .map((v) => convertAlternativeValue(true, v, getWeekdayMap()))
            .toList();
        return days.length == 1
            ? S.of(context).cronOnWeekday(days[0])
            : S.of(context).cronOnWeekdaysAnd(
                  days.getRange(0, days.length - 1).join(', '),
                  days.last,
                );
      case CronDayType.SPECIFIC_DAY_OF_MONTH:
        return ''; // Handle if required
      case CronDayType.LAST_DAY_OF_MONTH:
        return ''; // Handle if required
      case CronDayType.LAST_WEEKDAY_OF_MONTH:
        return S.of(context).cronOnLastXDayOfMonth(
            S.of(context).cronOnLastXDayOfMonth('weekday'));
      case CronDayType.LAST_X_DAY_OF_MONTH:
        return S.of(context).cronOnLastXDayOfMonth(lastDay ?? 'day');
      case CronDayType.DAY_BEFORE_END_OF_MONTH:
        return ''; // Handle if required
      case CronDayType.NEAREST_WEEKDAY_OF_MONTH:
        return ''; // Handle if required
      case CronDayType.XTH_DAY_OF_MONTH:
        return S.of(context).cronOnXthWeekdayOfMonth(
              serialNumberName(xthWeeks),
              convertAlternativeValue(true, xthWeekday, getWeekdayMap()),
            );
    }
  }
  ...
  Map<int, String> getWeekdayMap() {
    final List<String> dayNames = List.generate(7, (index) {
      // Use DateFormat to get the abbreviated weekday name based on the locale
      return DateFormat.E()
          .format(
            DateTime(2023, 1,
                index + 1), // Use the first week of January as reference
          )
  ...
  Map<int, String> getWeeksMap() {
    final List<String> weeks = [
      S.current.cronFirstWeek,
      S.current.cronSecondWeek,
      S.current.cronThirdWeek,
      S.current.cronFourthWeek,
      S.current.cronFifthWeek
    ];
