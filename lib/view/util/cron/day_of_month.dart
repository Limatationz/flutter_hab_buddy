...
class DayOfMonth extends CronEntity implements CronPart {
  ...
  String toReadableString(BuildContext context) {
    switch (cronDay.type) {
      case CronDayType.EVERY_WEEK:
        return ''; // Handle weekly logic if needed
      case CronDayType.EVERY_MONTH:
        return ''; // Handle monthly logic if needed
      case CronDayType.EVERY_START_AT_WEEK:
        return ''; // Handle weekly start logic if needed
      case CronDayType.EVERY_START_AT_MONTH:
        var startDay = everyStartDay ?? 0;
        return startDay > 0
            ? S.of(context).cronEveryDayStartingOn(everyDay, serialNumberName(startDay))
            : S.of(context).cronEveryDay(everyDay);
      case CronDayType.SPECIFIC_DAY_OF_WEEK:
        return '?'; // Handle specific day of the week logic if needed
      case CronDayType.SPECIFIC_DAY_OF_MONTH:
        var days = specificMonthDays.isEmpty ? [startIndex] : specificMonthDays;
        return days.length == 1
            ? S.of(context).cronOnDay(serialNumberName(days[0]))
            : S.of(context).cronOnDaysAnd(
          days.getRange(0, days.length - 1).join(', '),
          serialNumberName(days.last),
        );
      case CronDayType.LAST_DAY_OF_MONTH:
        return S.of(context).cronOnLastDayOfMonth(lastDay ?? 'last');
      case CronDayType.LAST_WEEKDAY_OF_MONTH:
        return S.of(context).cronOnLastWeekdayOfMonth;
      case CronDayType.LAST_X_DAY_OF_MONTH:
        return ''; // Implement if required
      case CronDayType.DAY_BEFORE_END_OF_MONTH:
        return S.of(context).cronOnDayBeforeEndOfMonth(dayBefore);
      case CronDayType.NEAREST_WEEKDAY_OF_MONTH:
        return S.of(context).cronOnNearestWeekdayOfMonth(serialNumberName(nearestWeekday));
      case CronDayType.XTH_DAY_OF_MONTH:
        return ''; // Implement if required
    }
  }
