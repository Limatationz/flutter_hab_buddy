...
class CronSecond extends CronEntity implements CronPart {
  ...
  bool get isSingleSpecific => specificSeconds.length == 1;
  ...
  String toReadableString(BuildContext context) {
    switch (type) {
      case CronSecondType.EVERY:
        return S.of(context).cronEverySecond;
      case CronSecondType.EVERY_START_AT:
        var startSecond = everyStartSecond ?? 0;
        return startSecond > 0
            ? S.of(context).cronEveryEverySecondsStartingAt(everySecond, startSecond)
            : S.of(context).cronEverySeconds(everySecond);
      case CronSecondType.SPECIFIC:
        var seconds = specificSeconds.isEmpty ? [startIndex] : specificSeconds;
        return seconds.length == 1
            ? S.of(context).cronAtSecond(seconds[0])
            : S.of(context).cronAtSecondsAnd(seconds.getRange(0, seconds.length - 2).join(', '),seconds.last);
      case CronSecondType.BETWEEN:
        return S.of(context).cronEverySecondBetweenAnd(betweenStartSecond, betweenEndSecond);
      case CronSecondType.NONE:
        return '';
    }
  }
